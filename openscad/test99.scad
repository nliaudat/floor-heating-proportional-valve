...
/*
$vpt =[23,4,19];//shows translation
$vpr = [80,0,115];// shows rotation
$vpd =165;//shows the camera distance
*/
...
/* [Valve] */
valve_pin_height = 5.0;//4.6;
//pin_height_adjust = 3.75;
//optional spring full compressed height
valve_spring_height = 5.0;


/* [M30x1.5 adapter] */
m30_adapter_wrench_size= 32;//36.0;
m30_adapter_radius = m30_adapter_wrench_size / sqrt(3.0);
m30_adapter_height=20.0;//6.0;
//optionnal spring height added
m30_adapter_tot_height = m30_adapter_height+ valve_spring_height;
...
/* [Slider spring (debug)]*/
spring_radius=2.5;//[1:.5:50]
spring_height=5.0;//[.5:.5:250]
spring_thickness=.4;//[.4:.1:3]
spring_density=750;//[20:1:750]
spring_ehanceSpring=1;//[1:1:4]

spring_nb360=spring_height/(360/spring_density);
spring_step=40/spring_radius;
...
motor_gap = 0.3;
motor_axle_lenght = 12.5; //must be less than sliding_cube_height
...
color("Green",0.6)scale([scale_factor,scale_factor,scale_factor])translate([-4070,3397,0])
    if(pcb_v2 == true){       
     ...
     //M30_adapter
       ...
       if (spring_enable == true){
           color("Red",0.25)
           translate([-0.5,-0.5,thread_height-top_thread_hang_height-0.75+sliding_cube[2]/2+spring_height/2-0.75])
           spring();
       }//spring
 ...
 //valve pin height  
  if(expand == 0){ //print|hole mode
      ...
      translate([0,0,-top_thread_hang_height-valve_pin_height/2-0.3])cylinder(r=thread_diam/2-cover_tickness, h=valve_pin_height-inner_cyl_extra_height/2);   
              ...
              //m3 hole
       translate([0,0,-(sliding_cube[2]-motor_axle_lenght)-1.5])    //todo : .5mm gap hardcoded
    cylinder(r=1.5+0.1, h=sliding_cube[2]);

    //M3 square nut : M3x7x2 metal 
        translate([0,5,sliding_cube[2]/2-3]) 
        cube(nut_size, center=true);
  }  //expand = 0
...
translate([-opto_sensor_placement[0]+opto_sensor_size[0]/2,-opto_sensor_placement[1]+opto_sensor_size[0]/2,sliding_cube[2]/2-3+thread_height-top_thread_hang_height])rotate([90,0,45+90+90])//cursor_V3();
   ...
   cube([opto_sensor_size[0]-1+expand*3,opto_sensor_size[1]-1+expand*3,cursor_height+valve_spring_height+expand*100], center=true);
        }//cursor cut
 
//add cut bottom expand #todo : not hardcode values
...
translate([-opto_sensor_placement[0]+opto_sensor_size[0]/2,-opto_sensor_placement[1]+opto_sensor_size[0]/2,sliding_cube[2]/2-3+thread_height-top_thread_hang_height])rotate([90,0,45+90+90])//cursor_V3();
     rotate([90,45,0])
    translate([-0.65,0,-cursor_height/2-top_thread_hang_height-valve_spring_height-1])
   cube([opto_sensor_size[0]+0.25,opto_sensor_size[1]+0.25,valve_spring_height], center=true);
...
////////////
// spring (https://www.thingiverse.com/thing:4134158)


//p1=[0,0,0];
module spring(){
for (i=[1:spring_step:360*spring_nb360]){
    computeAngle(i,i);
    if (spring_ehanceSpring>1)
        computeAngle(-i,i);
    if (spring_ehanceSpring>2)
        computeAngle(-180-i,i);
    if (spring_ehanceSpring>3)
        computeAngle(180+i,i);
}
}
...
//for (i=[0:-spring_step:-270])
//    computeAngleBase(i);

module line(start, end, spring_thickness = 1) {
    hull() {
        translate(start) cube(spring_thickness);
        translate(end) cube(spring_thickness);
    }
}

module computeAngle(angle,i){
    j=angle+(angle<0?spring_step:-spring_step);
    p1=[spring_radius*cos(j),spring_radius*sin(j),(i-spring_step)/spring_density];
    p2=[spring_radius*cos(angle),spring_radius*sin(angle),i/spring_density];
    line(p1,p2,spring_thickness);
}

module computeAngleBase(i){
    j=i+spring_step;
    p1=[spring_radius*cos(j),spring_radius*sin(j),0];
    p2=[radius*cos(i),radius*sin(i),0];
